...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const consulta = await prisma.consultas.findUnique({ where: { id: Number(params.id) } });
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const consulta = await prisma.consultas.update({ where: { id: Number(params.id) }, data });
    return NextResponse.json(consulta);
  } catch (error) {
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    await prisma.consultas.delete({ where: { id: Number(params.id) } });
