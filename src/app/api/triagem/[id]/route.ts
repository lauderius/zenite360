...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const triagem = await prisma.triagem.findUnique({ where: { id: Number(params.id) } });
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const triagem = await prisma.triagem.update({ where: { id: Number(params.id) }, data });
    return NextResponse.json(triagem);
  } catch (error) {
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    await prisma.triagem.delete({ where: { id: Number(params.id) } });
